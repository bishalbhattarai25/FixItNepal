...
import { Eye, EyeOff, ChevronLeft, Wrench, MapPin, FileText, User } from "lucide-react";
      ...
      <div className="w-full max-w-3xl">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
          ...
          <div className="mb-8 pb-6 border-b border-gray-100">
            ...
            <h1 className="text-2xl font-black text-gray-900 tracking-tight">Register as Mechanic</h1>
            <p className="text-gray-400 text-sm mt-1">Join as a professional mechanic and start earning</p>
          </div>
          ...
          <form onSubmit={f.handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="border border-gray-100 rounded-xl p-4 bg-gray-50">
                  <FileUploadField label="Profile Photo / Logo" type="Logo" fieldName="logoId" formik={f} />
                  <p className="text-xs text-gray-400 mt-2">JPG or PNG format</p>
                </div>
                <div className="border border-gray-100 rounded-xl p-4 bg-gray-50">
                  <FileUploadField label="Verification Document" type="Document" fieldName="documentId" formik={f} />
                  <p className="text-xs text-gray-400 mt-2">ID card or certificate (PDF)</p>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Field label="Full Name" error={f.touched.name && f.errors.name}>
                  <input placeholder="Your full name" className={`${inputBase} ${f.touched.name && f.errors.name ? inputError : ""}`} {...f.getFieldProps("name")} />
                </Field>
                <Field label="Email Address" error={f.touched.emailAddress && f.errors.emailAddress}>
                  <input type="email" placeholder="you@example.com" className={`${inputBase} ${f.touched.emailAddress && f.errors.emailAddress ? inputError : ""}`} {...f.getFieldProps("emailAddress")} />
                </Field>
            ...
            <button
              ...
              className="w-full bg-purple-600 hover:bg-purple-700 disabled:opacity-60 text-white py-3 rounded-xl font-semibold text-sm transition-colors flex items-center justify-center gap-2"
            >
              {loading ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : "Register as Mechanic"}
            </button>

            <p className="text-center text-sm text-gray-400">
              Already have an account?{" "}
              <button type="button" onClick={() => navigate("/login")} className="text-purple-600 font-semibold hover:underline underline-offset-2">
                Log in
              </button>
            </p>
