...
const ServiceLivetrack = () => {
  ...
  useLocationSender(connectionRef, requestId, serviceProviderId, !!requestId);
  ...
  useEffect(() => {
    const fetchRequest = async () => {
      ...
      try {
        const res = await instance.get(`/api/servicerequest/${requestId}`);
        setRequest(res.data);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    const fetchCustomer = async () => {
      ...
      try {
        const res = await instance.get(`/api/customer/${request.customerId}`);
  ...
  useEffect(() => {
    if (!requestId || !waitingCompletion) return;
  ...
  }, [waitingCompletion, requestId]);

  // 🚀 COMPLETION HANDLER (UPDATED)
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-800">
                  {distanceKm ? `${distanceKm} km away` : "Calculating..."}
                </p>
