...
import { ArrowLeft, CheckCircle, Clock, MapPin, Mail, Phone } from "lucide-react"; // Optional: lucide-react for icons
...
const ViewRecord = () => {
  ...
  if (!record) return <p className="p-6 text-center text-gray-500">No data found</p>;
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          disabled={record.status === 'Verified' || isVerifying}
        ...
        >
          {isVerifying ? "Processing..." : record.status === 'Verified' ? "Verified" : "Verify Mechanic"}
        </button>
      </div>
      ...
      <div className="bg-white border border-gray-100 shadow-xl shadow-gray-100/50 rounded-2xl overflow-hidden">
        ...
        <div className="px-8 pb-8">
          <div className="relative -mt-12 flex flex-col md:flex-row md:items-end md:space-x-5">
            <img
              src={record.logo?.accessUrl || "https://via.placeholder.com/150"}
              alt="logo"
              className="w-32 h-32 rounded-2xl border-4 border-white object-cover bg-white shadow-sm"
            />
            <div className="mt-4 md:mt-0 flex-1">
              <div className="flex items-center gap-3">
                <h1 className="text-3xl font-bold text-gray-900">{record.userName}</h1>
                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
                  record.status === 'Verified' ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"
                }`}>
                  {record.status || "Pending"}
                </span>
              </div>
              <p className="text-gray-500 flex items-center mt-1">
                <MapPin className="w-4 h-4 mr-1" /> {record.address?.city}, {record.address?.province}
              </p>
            </div>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                <p className="text-gray-700 leading-relaxed">
                  {record.address?.tole}<br />
                  {record.address?.city}, {record.address?.province}<br />
                  <span className="text-gray-400">Postal Code:</span> {record.address?.postalCode}
                </p>
              </div>
            </div>
          </div>
              ...
              {record.documents?.map((doc, idx) => (
                <div key={idx} className="group relative rounded-lg overflow-hidden border border-gray-200">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button className="text-white text-xs font-bold underline">View Full</button>
                  </div>
                </div>
              ))}
            </div>
