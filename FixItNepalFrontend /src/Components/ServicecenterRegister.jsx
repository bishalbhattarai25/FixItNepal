...
export const ServiceCenterRegister = () => {
  ...
  const handleChange = (e) => {
    ...
    // Check if the field should be a number (latitude/longitude)
    const isCoordinate = name.includes("latitude") || name.includes("longitude");
    const finalValue = isCoordinate ? parseFloat(value) || 0 : value;

    setFormData((prev) => {
      // Logic to handle up to 3 levels of nesting: address.locationCoordinatePoint.latitude
      if (keys.length === 1) {
        return { ...prev, [keys[0]]: finalValue };
      }
      if (keys.length === 2) {
        return {
          ...prev,
          [keys[0]]: { ...prev[keys[0]], [keys[1]]: finalValue },
        };
      }
      if (keys.length === 3) {
        return {
          ...
          [keys[0]]: {
            ...prev[keys[0]],
            [keys[1]]: { ...prev[keys[0]][keys[1]], [keys[2]]: finalValue },
          },
        };
      }
      return prev;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Log the actual validation error from the server
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2 bg-blue-50 p-6 rounded-2xl border border-blue-100 flex flex-col gap-4">
             ...
             <div className="grid grid-cols-2 gap-4">
                <input name="address.locationCoordinatePoint.latitude" type="number" step="any" placeholder="Latitude" className="bg-white p-2 rounded-lg border border-blue-100 text-sm" onChange={handleChange} />
                <input name="address.locationCoordinatePoint.longitude" type="number" step="any" placeholder="Longitude" className="bg-white p-2 rounded-lg border border-blue-100 text-sm" onChange={handleChange} />
             </div>
          </div>
        </div>
